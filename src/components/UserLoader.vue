  ...
  <div class="min-h-screen flex items-center justify-center bg-blue-900 text-white">
    ...
    <div v-else-if="userStore.isLoggedIn" class="text-center">
      ...
      <div class="bg-white/10 rounded-lg p-6 mb-6 backdrop-blur-sm mt-10">
        <div class="text-sm text-blue-200">Cuentas con {{ userStore.totalOwnedCards }} de {{ cardsDatabase.length }}
          estampas</div>
      </div>
...
onMounted(() => {
  ...
  if (!foundInUrl) {
    ...
    if (isIframe) {
      // If in URL, listen for postMessage from parent
      ...
      // Fallback to demo mode if no message received within 2 seconds
      setTimeout(() => {
        if (!customerIdReceived.value) {
          console.log('⏰ Timeout: No customer ID received, defaulting to demo mode.')
      ...
      // Not in an iframe and not in URL, default to demo immediately
      console.log('🚀 Standalone mode: Defaulting to demo.')
      apiService.setDemoMode(true)
      customerId = 'demo'
      customerIdReceived.value = true
      loadUserData()
