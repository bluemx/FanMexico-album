<template>
  <div>
    <!-- Trigger Button -->
    <button @click="openExchange" class="btn w-full">
      <Icon icon="mdi:swap-horizontal" class="w-4 h-4 mr-2" />
      Intercambios
    </button>

    <!-- Modal -->
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md"
      @click="close">

      <!-- Backdrop for type selector -->
      <div v-if="showTypeSelector" class="fixed inset-0 z-65" @click="showTypeSelector = false"></div>

      <div class="bg-white rounded-lg p-6 max-w-6xl max-h-[90vh] overflow-y-auto shadow-xl w-[95vw]" @click.stop>

        <!-- Header -->
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-bold text-pfblue">Intercambios de Estampas</h2>
          <div class="flex items-center gap-2">
            <button @click="refreshExchanges" :disabled="isRefreshing"
              class="btn btn-sm bg-pfblue hover:bg-pfblue/90 text-white disabled:opacity-50 disabled:cursor-not-allowed">
              <Icon :icon="isRefreshing ? 'mdi:loading' : 'mdi:refresh'" :class="{ 'animate-spin': isRefreshing }"
                class="w-4 h-4 mr-1" />
        ...
        <div v-if="myExchanges.length > 0" class="mb-8 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div class="space-y-3">
            <div v-for="exchange in myExchanges" :key="exchange.exchangeId"
              ...
              <button @click="cancelExchange(exchange.exchangeId)" :disabled="isCancelling === exchange.exchangeId"
                class="btn btn-sm bg-red-500 hover:bg-red-600 text-white disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
                <Icon :icon="isCancelling === exchange.exchangeId ? 'mdi:loading' : 'mdi:close-circle'"
                  :class="{ 'animate-spin': isCancelling === exchange.exchangeId }" class="w-4 h-4 mr-1" />
        ...
        <div class="mb-8">
          ...
          <div v-if="selectedOfferIdentifiers.length > 0" class="mb-3 flex flex-wrap gap-1">
            <span v-for="item in selectedOfferIdentifiers" :key="item.identifier"
          ...
          <div class="mb-4 flex gap-3">
            ...
            <div class="flex-1">
              ...
              <select v-model="selectedOfferCategory"
            ...
            <div class="w-48">
              ...
              <input v-model.number="selectedOfferNumber" type="number" min="1" placeholder="Número de estampa"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pfblue focus:border-transparent">
            </div>
          </div>

          <div v-if="availableStickers.length === 0" class="text-center py-8 text-gray-500">
          ...
          <div v-else-if="filteredAvailableStickers.length === 0" class="text-center py-8 text-gray-500">
          ...
          <div v-else class="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory">
            <div v-for="card in filteredAvailableStickers" :key="card.id"
              class="relative shrink-0 snap-start cursor-pointer flex flex-col items-center"
              :class="{ 'ring-4 ring-pfblue': isInOffer(card.id) }" @click="toggleOffer(card)">

              <div class="w-32 h-40 relative">
                <CardRenderer :iscard="true" :identifier="card.identifier ? Number(card.identifier) : 0"
                  :base="card.resource" :cardType="getCardTypeForRenderer(card)" />

                <!-- Selection Indicator -->
                <div v-if="isInOffer(card.id)"
                ...
                <div v-if="hasCardInAlbum(card.identifier)"
                  class="absolute top-2 left-2 bg-yellow-500 text-white text-xs px-2 py-1 rounded-full font-bold">
                  Repetida
                </div>
              </div>
        ...
        <div class="mb-8">
          ...
          <div class="mb-4">
            <div class="relative">
              ...
              <input v-model="searchQuery" type="text" placeholder="Buscar por número..."
          ...
          <div
            class="grid grid-cols-8 sm:grid-cols-10 md:grid-cols-12 lg:grid-cols-16 gap-2 max-h-64 overflow-y-auto p-2 bg-gray-50 rounded-lg">
            <button v-for="card in filteredCards" :key="card.identifier" @click="toggleWanted(card, $event)"
        ...
        <!-- Type Selector Popup -->
        <div v-if="showTypeSelector && typeSelectorCard"
          class="fixed z-70 bg-white rounded-lg shadow-2xl border-2 border-pfblue p-3 min-w-[180px]" :style="{
          ...
          <div class="flex flex-col gap-1.5">
            <button @click="selectWantedType('C')"
            ...
            <button v-if="typeSelectorCard.metal === 1" @click="selectWantedType('M')"
            ...
            <button v-if="typeSelectorCard.anim === 1" @click="selectWantedType('A')"
        ...
        <div class="flex gap-3 md:flex-row flex-col">
          <button @click="startExchange"
            ...
            <Icon :icon="isLoading ? 'mdi:loading' : 'mdi:swap-horizontal'" :class="{ 'animate-spin': isLoading }"
              class="w-5 h-5 mr-2" />
    ...
    <div v-if="showSuccessModal"
      class="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-md" @click="closeSuccess">

      <div class="bg-white rounded-lg p-8 max-w-2xl shadow-xl max-h-[80vh] overflow-y-auto" @click.stop>
        ...
        <p class="text-center text-gray-600 mb-6">
          Se encontraron {{ exchangeMatches.length }} socios(s) Fan México con las estampas que buscas.
          <br>
        ...
        <!-- Close Button -->
        <button @click="closeSuccess" class="btn w-full bg-green-500 hover:bg-green-600 text-white">
          Entendido
        </button>
      </div>
    </div>
    ...
    <!-- No Matches Modal -->
    <div v-if="showNoMatchesModal"
      class="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-md" @click="closeNoMatches">

      <div class="bg-white rounded-lg p-8 max-w-md shadow-xl" @click.stop>

        <!-- Close Button -->
        <div class="flex justify-end mb-2">
          <button @click="closeNoMatches"
            class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-gray-700 rounded-full hover:bg-gray-100">
            <Icon icon="mdi:close" class="w-6 h-6" />
          </button>
        </div>
        ...
        <p class="text-center text-gray-600 mb-6">
          No hay socios Fan México con las estampas que buscas en este momento.
          <br><br>
          Intenta con un intercambio diferente o vuelve más tarde.
        </p>

        <!-- Close Button -->
        <button @click="closeNoMatches" class="btn w-full bg-pfblue hover:bg-pfblue/90 text-white">
          Intentar de Nuevo
        </button>
      </div>
    </div>
    ...
    <div v-if="showCancelSuccessModal"
      ...
      <div class="bg-white rounded-lg p-8 max-w-md shadow-xl" @click.stop>
        ...
        <!-- Close Button -->
        <div class="flex justify-end mb-2">
          <button @click="closeCancelSuccess"
            class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-gray-700 rounded-full hover:bg-gray-100">
            <Icon icon="mdi:close" class="w-6 h-6" />
          </button>
        </div>
        ...
        <!-- Close Button -->
        <button @click="closeCancelSuccess" class="btn w-full bg-green-500 hover:bg-green-600 text-white">
          Entendido
        </button>
      </div>
    </div>
