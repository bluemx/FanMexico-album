  ...
  <div>
    ...
    <div class="flex flex-wrap justify-center gap-3 max-w-xl mx-auto">
      ...
      <template v-for="category in categoryDistribution">
        <button class="btn-outline grow" @click="navigateToFinal" v-if="category.categoryId === 12">Final</button>

        <button class="btn-outline grow" @click="navigateToCategory(category.categoryId)">{{ category.categoryName
        }}</button>
      </template>
